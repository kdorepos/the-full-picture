...
2. For EACH new episode (by its `index`), run the full pipeline per CLAUDE.md (review is required):
   a. Transcribe (paced, ~1hr for long ones; wait): python3 pipeline/the_full_picture.py https://feeds.megaphone.fm/the-big-picture --item <index>
   b. Read out/<slug>/transcript.txt; extract every film into web/src/data/episodes/<slug>.json per the schema and web/DESIGN.md. Set type (auction|list|discussion) by the episode's real shape. Find the Spotify id via the Spotify tool. Non-films -> excluded.
   c. python3 pipeline/enrich_tmdb.py web/src/data/episodes/<slug>.json
